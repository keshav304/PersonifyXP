...
import { orderCreated } from './handlers/orderCreated'
...
declare global {
  ...
  interface OrderCreated extends EventContext<Clients> {
    body: {
      domain: string
      orderId: string
      currentState: string
...
export default new Service({
  ...
  routes: {
    // `status` is the label/route ID from service.json. It maps to an array of middlewares (or a single handler).
    status: method({
      GET: [validate, status],
    }),
    catalog: method({
